# LLM-based Knowledge-Distillation Continual Learning for Adaptive Acoustic Sensing
...
(1) A re-implementation of the EquiAV model, configured for learning audio event classification for a distributed system in the home.
(2) A continual learning framwork in which an Audio LLM labels incoming data to use as training pairs for the EquiAV model to train on.
(3) A decision mechanism that chooses when to store incoming audio clip based on the observed data distribution.
