...
class Downloader:
    ...
    def get_audio_filename(self, ytid, start_seconds, end_seconds, ext):
        return f"{ytid}_{start_seconds}-{end_seconds}.{ext}"
    ...
    def download(
        ...
        :param max_labels_per_class: The maximum number of files to download for each specified class.
        """
        ...
        if self.labels is not None:
            ...
            # --- START of the new part to limit labels per class ---
            if max_labels_per_class is not None and max_labels_per_class > 0:
                print(f"Limiting to a maximum of {max_labels_per_class} files per class.")

                # Split entries with multiple labels into separate rows for each label
                metadata_exploded = metadata.copy()
                metadata_exploded['positive_labels'] = metadata_exploded['positive_labels'].str.split(',')
                metadata_exploded = metadata_exploded.explode('positive_labels')

                # Filter to only include the labels we care about and remove whitespace
                metadata_exploded['positive_labels'] = metadata_exploded['positive_labels'].str.strip()
                metadata_exploded = metadata_exploded[metadata_exploded['positive_labels'].isin(self.real_labels)]

                # Group by label and sample up to the max_labels_per_class limit
                sampled_metadata_list = []
                for label in self.real_labels:
                    label_data = metadata[metadata['positive_labels'].str.contains(label)]
                    if len(label_data) > max_labels_per_class:
                        sampled_data = label_data.sample(n=max_labels_per_class, random_state=42) # random_state for reproducibility
                        sampled_metadata_list.append(sampled_data)
                    else:
                        sampled_metadata_list.append(label_data)

                if sampled_metadata_list:
                    metadata = pd.concat(sampled_metadata_list)
                    # Deduplicate in case a single YTID was selected for multiple labels
                    metadata = metadata.drop_duplicates(subset=['YTID'])
        ...
        ext = self.get_ext(self.format)

        # Download the dataset
        joblib.Parallel(n_jobs=self.n_jobs, verbose=10, prefer="threads")(
        ...
        metadata['wav'] = metadata.apply(
            lambda row: os.path.join(
                self.root_path,
                save_folder_name,
                self.get_audio_filename(row['YTID'], row['start_seconds'], row['end_seconds'], ext)
            ),
            axis=1 # Apply the function row-wise
        )

        metadata['video_id'] = metadata.apply(
            lambda row: f"{row['YTID']}_{row['start_seconds']}-{row['end_seconds']}",
            axis=1
        )

        metadata['video_path'] = metadata.apply(
            lambda row: f"",
            axis=1
        )

        metadata['labels'] = metadata['positive_labels']


        file_exists_mask = metadata['wav'].apply(os.path.exists)

        # Filter the DataFrame, keeping only rows where the file exists
        initial_count = len(metadata)
        metadata = metadata[file_exists_mask]
        final_count = len(metadata)

        metadata_json_path = os.path.join(self.root_path, f"{save_folder_name}_metadata.json")
        final_json_output = {"data": metadata.to_dict(orient='records')}
        with open(metadata_json_path, 'w') as f:
            json.dump(final_json_output, f, indent=4)

        print(f"\nNumber of entries removed (files not found): {initial_count - final_count}")
        print(f"Final number of entries: {len(metadata)}")
    ...
    def download_file(
        ...
        file_path = os.path.join(
            self.root_path,
            save_folder_name,
            self.get_audio_filename(ytid, start_seconds, end_seconds, ext)
        )
