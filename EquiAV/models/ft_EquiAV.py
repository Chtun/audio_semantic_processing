...
class MainModel(nn.Module):
    ...
    def __init__(self, label_dim, num_mel_bins, drop_out, drop_path, train_mode="pretrained", img_size=224, target_length=1024, patch_size=16, in_chans=3,
                 ...
                 mlp_ratio=4., norm_layer=nn.LayerNorm, tr_pos=False, gpu=0, ftmode='multimodal', inter_linear=True, head_type='mlp', head_dim=512, **kwargs):
        super().__init__()
        if gpu == 0:
        ...
        if self.ftmode =='multimodal':
            ...
            if head_type == 'linear':
                self.mlp_head = nn.Sequential(nn.Linear(proj_out_dim, label_dim))
            elif head_type == 'mlp':
