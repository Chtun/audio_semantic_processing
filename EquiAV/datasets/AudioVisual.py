...
def wav2fbank(file_path: str, target_length: int, num_mel_bins: int):
        ...
        Args:
            file_path: The name of the main .wav audio file.
            target_length: The maximum length of the fbanks output tensor.
...
def format_model_input(file_path: str, target_length: int, num_mel_bins: int, norm_mean: float, norm_std: float, noise=False):
    # Get raw fbank (shape: target_length, num_mel_bins)
    fbank = wav2fbank(file_path=file_path, target_length=target_length, num_mel_bins=num_mel_bins)

    # Apply the transposes to format data properly
    fbank = torch.transpose(fbank, 0, 1) # (num_mel_bins, target_length)
    fbank = fbank.unsqueeze(0)
    fbank = fbank.squeeze(0)             # (num_mel_bins, target_length)
    fbank = torch.transpose(fbank, 0, 1) # (target_length, num_mel_bins)

    # Now, add the channel dimension (3 channels) as seen in __getitem__
    fbank = fbank.unsqueeze(0).repeat(3, 1, 1) # (3, target_length, num_mel_bins)

    # Normalize the input
    fbank = (fbank - norm_mean) / (norm_std)

    # Apply noise if needed (usually skipped for inference)
    if noise == True:
        fbank = torch.roll(fbank, np.random.randint(-target_length, target_length), 0)

    # The model expects a BATCH dimension at the very beginning
    fbank = fbank.unsqueeze(0) # (1, 3, target_length, num_mel_bins) - This is your 'a' for forward_feat_audio_only
...
class MainDataset(Dataset):
    ...
    def get_raw_item(self, index):
        ...
        if random.random() < self.mixup:
            ...
            if self.ftmode != "audio_only":
                ...
                image = torch.zeros([3, self.im_res, self.im_res]) + 0.01
                raw_image = torch.zeros([3, self.im_res, self.im_res]) + 0.01
    ...
    def __getitem__(self, index):
        ...
        if self.ftmode != "audio_only":
            ...
            t_v = torch.zeros([3, self.im_res, self.im_res])
            image_aug = torch.zeros([3, self.im_res, self.im_res])
        ...
        if self.cen_mean:
            ...
            if self.ftmode != "audio_only":
                ...
                t_v_list = torch.zeros([3, self.im_res, self.im_res])
